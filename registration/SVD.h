#ifndef SVD_H
#define SVD_H

#include <stdio.h>
#include <math.h>
#include <stdlib.h>

#define NR_END 1
#define FREE_ARG char*


  static  float SQR(float a){
    return (a == 0.0 ? 0.0f : a*a);
  }
  
  static  float FMAX(float a, float b){
    return (a > b ? a : b);
  }
  
  static  int IMIN(int a, int b){
    return (a < b ? a : b);
  }
  
  static  float SIGN(float a, float b){
    return (b >= 0.0 ? (float)fabs(a) : -(float)fabs(a));
  }
  


  static  float pythag(float a, float b){
    float absa,absb;
    absa=(float)fabs(a);
    absb=(float)fabs(b);
    if (absa > absb) return absa*(float)sqrt(1.0f+SQR(absb/absa));
    else return (absb == 0.0 ? 0.0f : absb*(float)sqrt(1.0f+SQR(absa/absb)));
  }
  
  static void nrerror(const char error_text[]){
    /* Numerical Recipes standard error handler */
    fprintf(stderr,"Numerical Recipes run-time error...\n");
    fprintf(stderr,"%s\n",error_text);
    fprintf(stderr,"...now exiting to system...\n");
    //exit(1);
  }

  static  float *vector(long nl, long nh){
    /* allocate a float vector with subscript range v[nl..nh] */
    float *v;
    
    v=(float *)malloc((size_t) ((nh-nl+1+NR_END)*sizeof(float)));
    if (!v) nrerror("allocation failure in vector()");
    return v-nl+NR_END;
  }
  
  static  void free_vector(float *v, long nl, long nh){
    /* free a float vector allocated with vector() */
    free((FREE_ARG) (v+nl-NR_END));
  }
  
  static int svdcmp(double **a, int m, int n, double w[], double **v){
    int flag,i,its,j,jj,k,l,nm;
    float anorm,c,f,g,h,s,scale,x,y,z,*rv1;
    
    rv1=vector(1,n);
    g=scale=anorm=0.0;
    for (i=1;i<=n;i++) {
      l=i+1;
      rv1[i]=scale*g;
      g=s=scale=0.0;
      if (i <= m) {
        for (k=i;k<=m;k++) scale += (float)fabs(a[k][i]);
        if (scale) {
          for (k=i;k<=m;k++) {
            a[k][i] /= scale;
            s += (float)(a[k][i]*a[k][i]);
          }
          f=(float)a[i][i];
          g = -SIGN((float)sqrt(s),f);
          h=f*g-s;
          a[i][i]=f-g;
          for (j=l;j<=n;j++) {
            for (s=0.0,k=i;k<=m;k++) s += (float)(a[k][i]*a[k][j]);
            f=s/h;
            for (k=i;k<=m;k++) a[k][j] += f*a[k][i];
          }
          for (k=i;k<=m;k++) a[k][i] *= scale;
        }
      }
      w[i]=scale *g;
      g=s=scale=0.0;
      if (i <= m && i != n) {
        for (k=l;k<=n;k++) scale += (float)fabs(a[i][k]);
        if (scale) {
          for (k=l;k<=n;k++) {
            a[i][k] /= scale;
            s += (float)(a[i][k]*a[i][k]);
          }
          f=(float)a[i][l];
          g = -SIGN((float)sqrt(s),f);
          h=f*g-s;
          a[i][l]=f-g;
          for (k=l;k<=n;k++) rv1[k]=(float)(a[i][k]/h);
          for (j=l;j<=m;j++) {
            for (s=0.0,k=l;k<=n;k++) s += (float)(a[j][k]*a[i][k]);
            for (k=l;k<=n;k++) a[j][k] += s*rv1[k];
          }
          for (k=l;k<=n;k++) a[i][k] *= scale;
        }
      }
      anorm=FMAX(anorm,(float)(fabs(w[i])+fabs(rv1[i])));
    }
    for (i=n;i>=1;i--) {
      if (i < n) {
        if (g) {
          for (j=l;j<=n;j++)
            v[j][i]=(a[i][j]/a[i][l])/g;
          for (j=l;j<=n;j++) {
            for (s=0.0,k=l;k<=n;k++) s += (float)(a[i][k]*v[k][j]);
            for (k=l;k<=n;k++) v[k][j] += s*v[k][i];
          }
        }
        for (j=l;j<=n;j++) v[i][j]=v[j][i]=0.0;
      }
      v[i][i]=1.0;
      g=rv1[i];
      l=i;
    }
    for (i=IMIN(m,n);i>=1;i--) {
      l=i+1;
      g=(float)w[i];
      for (j=l;j<=n;j++) a[i][j]=0.0;
      if (g) {
        g=1.0f/g;
        for (j=l;j<=n;j++) {
          for (s=0.0,k=l;k<=m;k++) s += (float)(a[k][i]*a[k][j]);
          f=(float)((s/a[i][i])*g);
          for (k=i;k<=m;k++) a[k][j] += f*a[k][i];
        }
        for (j=i;j<=m;j++) a[j][i] *= g;
      } else for (j=i;j<=m;j++) a[j][i]=0.0;
      ++a[i][i];
    }
    for (k=n;k>=1;k--) {
      for (its=1;its<=30;its++) {
        flag=1;
        for (l=k;l>=1;l--) {
          nm=l-1;
          if ((float)(fabs(rv1[l])+anorm) == anorm) {
            flag=0;
            break;
          }
          if ((float)(fabs(w[nm])+anorm) == anorm) break;
        }
        if (flag) {
          c=0.0;
          s=1.0;
          for (i=l;i<=k;i++) {
            f=s*rv1[i];
            rv1[i]=c*rv1[i];
            if ((float)(fabs(f)+anorm) == anorm) break;
            g=(float)w[i];
            h=pythag(f,g);
            w[i]=h;
            h=1.0f/h;
            c=g*h;
            s = -f*h;
            for (j=1;j<=m;j++) {
              y=(float)a[j][nm];
              z=(float)a[j][i];
              a[j][nm]=y*c+z*s;
              a[j][i]=z*c-y*s;
            }
          }
        }
        z=(float)w[k];
        if (l == k) {
          if (z < 0.0) {
            w[k] = -z;
            for (j=1;j<=n;j++) v[j][k] = -v[j][k];
          }
          break;
        }
        if (its == 30) return -1;
        x=(float)w[l];
        nm=k-1;
        y=(float)w[nm];
        g=rv1[nm];
        h=rv1[k];
        f=((y-z)*(y+z)+(g-h)*(g+h))/(2.0f*h*y);
        g=pythag(f,1.0);
        f=((x-z)*(x+z)+h*((y/(f+SIGN(g,f)))-h))/x;
        c=s=1.0;
        for (j=l;j<=nm;j++) {
          i=j+1;
          g=rv1[i];
          y=(float)w[i];
          h=s*g;
          g=c*g;
          z=pythag(f,h);
          rv1[j]=z;
          c=f/z;
          s=h/z;
          f=x*c+g*s;
          g = g*c-x*s;
          h=y*s;
          y *= c;
          for (jj=1;jj<=n;jj++) {
            x=(float)v[jj][j];
            z=(float)v[jj][i];
            v[jj][j]=x*c+z*s;
            v[jj][i]=z*c-x*s;
          }
          z=pythag(f,h);
          w[j]=z;
          if (z) {
            z=1.0f/z;
            c=f*z;
            s=h*z;
          }
          f=c*g+s*y;
          x=c*y-s*g;
          for (jj=1;jj<=m;jj++) {
            y=(float)a[jj][j];
            z=(float)a[jj][i];
            a[jj][j]=y*c+z*s;
            a[jj][i]=z*c-y*s;
          }
        }
        rv1[l]=0.0;
        rv1[k]=f;
        w[k]=x;
      }
    }
    free_vector(rv1,1,n);
    
    return 0;
  }
  
  static void svbksb(double **u, double w[], double **v, int m, int n, double b[], double x[]){
    int jj,j,i;
    float s,*tmp;
    
    tmp=vector(1,n);
    for (j=1;j<=n;j++) {
      s=0.0;
      if (w[j]) {
        for (i=1;i<=m;i++) s += (float)(u[i][j]*b[i]);
        s /= (float)w[j];
      }
      tmp[j]=s;
    }
    for (j=1;j<=n;j++) {
      s=0.0;
      for (jj=1;jj<=n;jj++) s += (float)(v[j][jj]*tmp[jj]);
      x[j]=s;
    }
    free_vector(tmp,1,n);
  }

  /* (C) Copr. 1986-92 Numerical Recipes Software 9z!+!1(t+%. */


#undef NR_END
#undef FREE_ARG

#endif
