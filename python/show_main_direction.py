...
with open('/home/tomokimori/CLionProjects/3dreconGPU/volume_bin/cfrp_xyz7_13axis/pca/cest_1234567cond_md1_196x196x196.raw') as f:
    u_raw = np.fromfile(f, dtype=np.float32)

with open('/home/tomokimori/CLionProjects/3dreconGPU/volume_bin/cfrp_xyz7_13axis/pca/cest_1234567cond_md2_196x196x196.raw') as f:
    v_raw = np.fromfile(f, dtype=np.float32)

with open('/home/tomokimori/CLionProjects/3dreconGPU/volume_bin/cfrp_xyz7_13axis/pca/cest_1234567cond_md3_196x196x196.raw') as f:
    w_raw = np.fromfile(f, dtype=np.float32)

num_voxel = 196 # voxel suu
bin = 3
...
ax_fiber = fig.add_subplot(1, 1, 1, projection='3d', facecolor='white')
ax_sphere = fig.add_subplot(5, 5, 16, projection='3d', facecolor='white')
def update(i):
    # 前フレームのグラフを初期化
    plt.cla()
    ...
    ax_fiber.set_facecolor('white')

    ax_fiber.w_xaxis.set_pane_color((1., 1., 1., 1.))
    ax_fiber.w_yaxis.set_pane_color((1., 1., 1., 1.))
    ax_fiber.w_zaxis.set_pane_color((1., 1., 1., 1.))
    ...
    """
    ax_sphere.w_xaxis.set_pane_color((1., 1., 1., 1.))
    ax_sphere.w_yaxis.set_pane_color((1., 1., 1., 1.))
    ax_sphere.w_zaxis.set_pane_color((1., 1., 1., 1.))
    """
    ax_sphere.axis("off")
    end = 0.5
...
update(4)
plt.show()
fig.savefig('/home/tomokimori/Pictures/3dview/cfrp_cest_with_background_white4.png')

update(7)
# fig.savefig('/home/tomokimori/Pictures/3dview/cfrp_cest_with_background_white7.png')
"""
ani = FuncAnimation(fig=fig, func=update, frames=frame_num, interval=800)
ani.save('gfrp_tw_with_sphere.gif')
"""
