...
int main() {
    ...
    sinogram.load("../volume_bin/cube_proj_phantom-100x100x100.raw", NUM_DETECT_U, NUM_DETECT_V, NUM_PROJ);
    ...
    bool rotate = true;
    reconstruct(sinogram, ct, geom, 1, 10, rotate);
    ...
    std::string savefilePath1 =
            "../volume_bin/cube_proj_cuda-" + std::to_string(NUM_DETECT_U) + "x" + std::to_string(NUM_DETECT_V) + "x" +
