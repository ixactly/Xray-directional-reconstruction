...
void flipAxis(Volume<float> *dst, const Volume<float> *ct, int size_x, int size_y, int size_z) {
    for (int x = 0; x < size_x; x++) {
        for (int y = 0; y < size_y; y++) {
            for (int z = 0; z < size_z; z++) {
                const float I[4] = {ct[0](x, y, z), ct[1](x, y, z), ct[2](x, y, z), ct[3](x, y, z)};
                ...
                // r, g, b
                /*
                dst[0](x, z, y) = a * colorIntensity(phi, 0.0f);
                // dst[0](x, z, y) = a * ct[0](x, y, z);
                dst[1](x, z, y) = a * colorIntensity(phi, M_PI / 3.0f);
                dst[2](x, z, y) = a * colorIntensity(phi, 2.0f * M_PI / 3.0f);
                dst[3](x, z, y) = phi;
                */
